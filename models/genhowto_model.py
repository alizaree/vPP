...
class MyPipe:
    ...
    def encode_tstate(self, prompt_s, tokenizer):
        with torch.no_grad():
            ...
            tstate_embds = last_hidden_states[eos_positions[:, 0], eos_positions[:, 1]].view(
                self.args.batch_size, len(prompt_s)//self.args.batch_size, -1)
        return tstate_embds

    def encode_actions(self, prompt_a, tokenizer):
        with torch.no_grad():
            ...
            action_embds = last_hidden_states[eos_positions[:, 0], eos_positions[:, 1]].view(
                self.args.batch_size, len(prompt_a)//self.args.batch_size, -1)
        return action_embds
